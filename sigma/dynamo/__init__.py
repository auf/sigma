...
class PropertyRegisty(object):
    ...
    def synchro_meta_2_instance(self):
        # Loop in all registered chain of dynamic properties
        for connection in dynamo_registry.registry:
            ...
            instances = instance_model.objects.all()

            logger.info("cmd: sync %s according %s" % (instance_model.__name__,
                meta_model.__name__))
            logger.info("cmd: %s instances to process" % len(instances))
            ...
            for instance in instances:
                ...
                instance_properties = getattr(instance, "%s_set" % f_inst_properties).\
                        select_related('type').\
                        all()
                ...
                for p in instance_properties:
                    if p.type not in meta_properties:
                        p.delete()
            logger.info("cmd: %s instances processed." % len(instances))
